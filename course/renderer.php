    ...
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
        ...
        if ($chelper->get_show_courses() < self::COURSECAT_SHOW_COURSES_EXPANDED) {
            if ($course->has_summary() || $course->has_course_contacts() || $course->has_course_overviewfiles()) {
                ...
                $my_url_text = "Знакомство с курсом";
				$content .= html_writer::link($url, $my_url_text, array('class' => 'info-about-course'));
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course) {
        ...
        if ($course->has_summary()) {
			...
			if (!is_enrolled($context)) { 
				$my_form_action = new moodle_url('/form/index.php');
				$send_button_value = "Оставить заявку на обучение";
				$get_course_name = $chelper->get_course_formatted_name($course);
				$content .= html_writer::start_tag('form', array('action' => $my_form_action, 'method' => 'post'));
				$content .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sent_course', 'value' => $get_course_name));
				$content .= html_writer::empty_tag('input', array('type' => 'submit', 'value' => $send_button_value));
				$content .= html_writer::end_tag('form');	
			} else {
				$content .= '<a href="' . $CFG->wwwroot . '/course/view.php?id=' . $course->id . '" class="link-like-button">Начать обучение</a>';				
			}
			/* my code */

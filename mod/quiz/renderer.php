...
class mod_quiz_renderer extends plugin_renderer_base {
    ...
    public function review_summary_table($summarydata, $page) {
		...
		/* my code */
		if (isset($_POST['hide_answers'])) {
			$output .= html_writer::start_tag('div', array('class' => 'hide-answers'));
		}		
		
		$context = context_system::instance();
		if (has_capability('moodle/course:visibility', $context)) {
			$output .= html_writer::start_tag('form', array('method' => 'post', 'class' => 'right-align'));
			$output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'hide_answers', 'value' => 'Скрыть ответы'));
    ...
    public function review_next_navigation(quiz_attempt $attemptobj, $page, $lastpage, $showall = null) {
		...
		if (has_capability('moodle/course:viewhiddencourses', $context)) {
			...
			$nav = '';
			$nav .= '<div class="students-finish-review-link"><strong>' . $this->finish_review_link($attemptobj) . '</strong></div>';
			return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
		}
    }
    ...
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free') {
        ...
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'previous',
                    'value' => get_string('navigateprevious', 'quiz'), 'class' => 'float-left'));
        }
        /*if ($lastpage) {
        ...
        if (!$lastpage) {
			...
			$nextlabel = get_string('endtest', 'quiz');
			$output .= html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'next',
            'value' => $nextlabel, 'class' => 'end-test-button'));
			$output .= html_writer::start_tag('div', array('class' => 'if-all-answered'));
			$output .= "Если Вы ответили на все вопросы, нажмите на кнопку &laquo;Закончить тест&raquo;";
			$output .= html_writer::end_tag('div');
		}
        $output .= html_writer::end_tag('div');
    ...
    public function view_table($quiz, $context, $viewobj) {
        ...
        if ($viewobj->markcolumn) {
            $table->head[] = get_string('marks', 'quiz') . ' (макс. ' .
                    quiz_format_grade($quiz, $quiz->sumgrades) . ')';
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->gradecolumn) {
            $table->head[] = get_string('grade') . ' (макс. ' .
		...
		$send_button_value = "Отправить результаты в учебный центр";
		$output .= html_writer::start_tag('form', array('style' => 'text-align: right;', 'method' => 'post'));
		$output .= html_writer::empty_tag('input', array('name' => 'send_result', 'type' => 'submit', 'value' => $send_button_value));
		$output .= html_writer::end_tag('form');	
		...
		if (isset($_POST['send_result'])) {
			...
			$stmt = mysqli_prepare($link, "SELECT attempt, state, timestart, timefinish, sumgrades FROM mdl_quiz_attempts WHERE userid=? AND quiz=?");
			...
			foreach ($result as $data){
				if ($data['state'] == "inprogress") {
					$output .= '<div class="results-page-messages">Внимание! Необходимо завершить последнюю попытку перед отправкой результатов.</div>';
					$test_inprogress = "true";
				}
			}
			
			if(!isset($test_inprogress)) {
				$to = "majorov_sv@skc-fmba.ru, kryilova_oo@skc-fmba.ru";
				$subject = $USER->firstname . " отправил результаты тестирования";
				$message = "
					...
					<table border='1'>
						<tbody>
							<tr>
								<td>Попытка</td>
								<td>Дата, время начала</td>
								<td>Дата, время завершения</td>
								<td>Баллы</td>
								<td>Оценка</td>
							</tr>";
				foreach ($result as $data){
					$message .= "
						<tr>
							<td>" . $data['attempt'] . "</td>
							<td>" . date('d.m.Y H:i', $data['timestart']) . "</td>
							<td>" . date('d.m.Y H:i', $data['timefinish']) . "</td>
							<td>" . round($data['sumgrades'], 0) . "</td>
							<td>" . ($data['sumgrades'] * 100) / $quiz->sumgrades . "%</td>							
						</tr>";
				}
				$message .= "</tbody></table>";
		
				if (mail($to, "$subject", "$message", "Content-type:text/html; charset = utf-8\r\nFrom:Дистанционное обучение")) {
					$output .= '<div class="results-page-messages">Результаты отправлены.</strong></div>';
				} else {
					$output .= '<div class="results-page-messages">К сожалению, во время отправки произошла ошибка. Попробуйте, пожалуйста, позже. Либо сообщите о проблеме по электронной почте: kryilova_oo@skc-fmba.ru</div>';
				}
			}
